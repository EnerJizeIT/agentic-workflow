...
**awf** добавляет структуру: supervisor-агент планирует работу, worker-агенты выполняют через pipeline (аналитик → архитектор → разработчик → QA → аудит), и ты одобряешь каждый результат перед коммитом. Всё через естественный язык — *"начни работу по бэклогу"*, *"проверь и одобри"*, *"отклони — DOMParser не починен"*.
