...
### RUN6-2026-09-22 · Фидбек с аудита topic-trainer (6 задач, per-item pipelines, awf 1.2.0)

**Источник:** 4 отчёта `awf feedback` на рабочем столе (22.09). Боевой забег «шесть слоёв» сработал: 6/6 approve, у каждой задачи свой пайплайн.

⬜ **#1 BUG: `done`-событие не приходит после approve.** Пайплайн коммитит и завершается, но `awf_wait_for_event(actionable_only=True)` отдаёт timeout с ПУСТЫМ state (stage/todo = null); супервизор проверяет git log, чтобы понять «можно run_next» (5/5 раз). Чинить: детект завершения (state очищен / процесс вышел) → событие `done` в течение секунд после approve.
⬜ **#2 BUG: ложная тревога `conflict_warning` в забеге.** `awf_status` пишет «N active TODOs… others are stale — rollback or reset» на нормальную очередь забега (элементы ждут хода, todo_id пинится run_next). В активном забеге — молчать или «очередь: N элементов ждут хода».
⬜ **#3 Cap ожидания честный.** `suggested_timeout` зашит 55с (`TRANSPORT_CAP`), хотя клиентский таймаут 600000 мс; в `next_action` каждый раз совет «подними mcp timeout» (уже поднят); эмпирический предел ~300с. Читать фактический клиентский таймаут (или принимать параметром), не советовать сделанное.
⬜ **#4 `awf todo-update <id>`.** Обновить содержимое существующего TODO, СОХРАНИВ номер (+обновить baseline/ready), либо `dispatch --force` с бэкапом прежнего .md. Сейчас обход — retire + перевыпуск под новым номером (номер «едет», в STATUS уже упоминался).
⬜ **#5 MCP-тул `awf_tree_sha`.** Verify-ритуал требует tree-sha перед approve, но он только в CLI — в MCP-сессии приходится звать bash (выпадает из типизированного контура).
⬜ **#6 Онбординг супервизора без подсказок владельца.** (а) строка в системный промпт плагина: «начни с `awf_brief`» (сейчас «jump straight to awf_status»); (б) дефолты в бриф: чекпоинт плана включён по умолчанию и как пропустить; «пайплайн завершён» = state очищен + коммит `awf(verify)`; `run_next` ждёт finished; несколько активных TODO в забеге — норма; ссылка на USAGE.md; (в) по строке «когда использовать» на семейства тулов; (г) пять сценариев с точными шагами (одиночная задача, забег, verify-ритуал, blocked, salvage); (д) доктрина супервизора как глобальный SKILL «awf-supervisor». **Критерий:** новая сессия супервизора проводит забег без подсказок владельца и без чтения исходников awf.
⬜ **#7 BUG: дашборд рисует чужой пайплайн («путает роли»).** Наблюдение владельца + репро супервизора: забег с per-item пайплайном (`audit-llm`) — дашборд показывает стадии/роли АКТИВНОГО (дефолтного) пайплайна (`implement/qa` вместо `llm-audit/agent-security-auditor`). Причина: `awf/api/dashboard.py` зовёт `resolve_pipeline_file(project_dir)` без имени; имя фактически запущенного пайплайна движок в `pipeline_state` НЕ пишет (поля `pipeline` там нет). Чинить: движок сохраняет имя пайплайна в state (и чистит на выходе); дашборд берёт цепочкой `state.pipeline → pipeline элемента забега → front-matter TODO → config default`, рисует стадии оттуда и показывает имя пайплайна чипом. Тесты: репро (audit-llm vs default), одиночный `--pipeline X`, фолбэк, пустое состояние.
